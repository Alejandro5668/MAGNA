...
## DEC-081 — Protocolo QA inyectado por heurística + `qa_verified` como ground truth de campo

**Decisión:** `aicli/services/qa_protocol.py` (nuevo, stdlib-only) define `is_bug_task(desc, jira_data)`
— heurística de palabras clave en español, con acentos normalizados y límites de palabra — y
`QA_PROTOCOL`/`render_qa_protocol(project_id)`, un bloque de instrucciones fijo que exige plan de
pruebas previo, reproducción independiente, verificación en navegador real (`claude-in-chrome`) y
en base de datos, producción de solo lectura salvo autorización explícita, y un tope de 3 ciclos de
fix + verificación antes de escalar. `_execute_task` (`task.py:288`, único call site) calcula
`es_bug` y lo pasa a `build_context(..., es_bug=es_bug)`, que antepone el protocolo como primer
fragmento cuando `es_bug=True`.

Al cerrar la verificación, la sesión de Claude escribe `~/.mycontext/qa_results/<TICKET>.json` con
`{"verified": bool, "attempts": int, ...}`. `ctx sync` lo lee con la nueva `read_qa_result()`
(`tickets.py`) — que borra el archivo tras leerlo, sea el JSON válido o corrupto — y lo pasa a
`save_round(..., qa_verified=qa_verified)`. `qa_verified` es siempre `None` (nunca `False`) si el
archivo no existe, está corrupto, o `verified` no es un `bool` real: `False` significa "se verificó
y falló", `None` significa "no sabemos", y una sesión crasheada no debe ensuciar la métrica de tasa
de reapertura que esta feature existe para habilitar.

**Por qué:** Antes de esto, una tarea marcada como bug recibía exactamente el mismo contexto que
cualquier otra — nada empujaba a Claude a verificar en navegador real ni a limitar los reintentos.
El resultado de esa verificación tampoco quedaba registrado en ningún lado: no había forma de medir,
por ticket, si lo que se marcó como resuelto de verdad se verificó. `qa_protocol.py` es stdlib puro
(sin dependencia nueva) y ambos parámetros nuevos (`es_bug`, `qa_verified`) son keyword-defaulted,
así que ningún caller existente se rompe. `is_bug_task` es una función pura sin estado de módulo
porque `_execute_task` corre en un `ThreadPoolExecutor` de fondo en el flujo del TUI.

**Alternativas descartadas:**
- Clasificación de bug vía IA (llamada a Claude) en vez de heurística de palabras clave: costo,
  latencia y no-determinismo explícitamente fuera de alcance de la propuesta.
- Guardar el resultado de verificación en `tickets/<TICKET>.qa_result.json`: `load_tickets()` hace
  `glob("*.json")` sobre ese directorio y lo tomaría como un ticket fantasma; un directorio hermano
  `qa_results/` elimina la colisión por construcción.
- Dejar el archivo de resultado sin borrar tras leerlo: una ronda o reapertura posterior volvería a
  leer un resultado viejo — una mentira silenciosa en el campo cuyo propósito es la verdad de campo.
- Texto del protocolo con `<project_id>` fijo o vía `str.format`: la ruta no se puede derivar del
  archivo de contexto (que no lleva `project_id`), y el bloque contiene un ejemplo JSON con llaves
  literales que `.format()` rompería.
