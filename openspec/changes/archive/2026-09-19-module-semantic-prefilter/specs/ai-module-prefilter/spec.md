...
WHEN a project has more than 20 documented modules and the vector
collection is available, `_detect_relevant_modules` MUST query the
collection for the 20 modules most semantically similar to `task_desc`
and MUST build the prompt's module listing using only those modules,
unioned with any module pinned via `--file`. It MUST NOT send the full
module catalog in this case.

#### Scenario: Large project sends only top-20 plus pinned module
- GIVEN a project with 150 documented modules and no `--file` argument
- WHEN `ctx task` runs with a given `task_desc`
- THEN the prompt's module listing contains at most 20 modules
...
- WHEN `ctx task` runs
- THEN the pinned module is present in the final listing in addition to the top-20 results
- AND the caller's original `modules` list is not mutated

### Requirement: Graceful Degradation

IF a project has 20 or fewer documented modules, OR the Chroma
collection or query fails for any reason, `_detect_relevant_modules`
MUST fall back to the full unfiltered module list and MUST NOT raise an
error or block `ctx task` execution.

#### Scenario: Small project skips prefilter
- GIVEN a project with 15 documented modules
- WHEN `ctx task` runs
- THEN the full list of 15 modules is used without querying Chroma
...
- WHEN `ctx task` runs
- THEN `_detect_relevant_modules` proceeds using the full unfiltered module list
