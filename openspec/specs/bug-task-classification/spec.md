# Capability: bug-task-classification

Keyword-heuristic classification of bug-shaped tasks — `is_bug_task(desc, jira_data)` pure function, Spanish keywords over description/Jira summary/description, reopen marker as positive signal, recall-biased, threads into `build_context(es_bug=True)` from `_execute_task` only, byte-identical output for non-bug paths.
