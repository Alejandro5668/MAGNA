# Capability: qa-outcome-tracking

Ground-truth QA outcome measurement sourced from the session itself — session-produced `{verified: bool, attempts: int}` artifact, `ctx sync` reads and passes through to `save_round(qa_verified=...)`, stored in `ronda` dict, defaults to `None` when unknown, never AI-inferred.
