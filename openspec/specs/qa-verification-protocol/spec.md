# Capability: qa-verification-protocol

The fixed verification contract injected into bug-shaped sessions — requires Claude to write an explicit test plan (happy path + identified edge cases/roles/data variants) before executing, reproduce independently, verify in a real browser inspecting console/network/DB, read-only in production unless explicitly authorized, pause when data is missing, retry with new evidence up to 3 attempts with escalation after cap, never declare resolved without verification.
